...
module.exports = class extends Command {
    constructor(...args) {
        super(...args, {
            name: "fbi",
            description: "Play the fbi open up sound effect",
            usage: "fbi",
            category: "Soundboard"
        });
    }
        ...
        try {
            ...
            channel.join().then((connection) => {
                const dispatcher = connection.play(
                    `${process.cwd()}/soundboard_sounds/fbi-open-up.mp3`
                );

                message.channel.send(
                    `${this.client.emoji.playing} | Now playing the fbi sound effect in voice channel \`${connection.channel.name}\``
                );
