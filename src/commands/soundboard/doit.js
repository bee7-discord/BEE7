...
module.exports = class extends Command {
    constructor(...args) {
        super(...args, {
            name: "doit",
            description: "Play the just do it sound effect",
            usage: "doit",
            category: "Soundboard"
        });
    }
        ...
        try {
            ...
            channel.join().then((connection) => {
                const dispatcher = connection.play(
                    `${process.cwd()}/soundboard_sounds/just-do-it.mp3`
                );

                message.channel.send(
                    `${this.client.emoji.playing} | Now playing the just do it sound effect in voice channel \`${connection.channel.name}\``
                );
