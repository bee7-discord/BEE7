...
module.exports = class extends Command {
    constructor(...args) {
        super(...args, {
            name: "oof",
            description: "Play the oof sound effect",
            usage: "oof",
            category: "Soundboard"
        });
    }
        ...
        try {
            ...
            channel.join().then((connection) => {
                const dispatcher = connection.play(
                    `${process.cwd()}/soundboard_sounds/minecraft-oof.mp3`
                );

                message.channel.send(
                    `${this.client.emoji.playing} | Now playing the oof sound effect in voice channel \`${connection.channel.name}\``
                );
