...
module.exports = class extends Command {
    constructor(...args) {
        super(...args, {
            name: "illuminati",
            description: "Play the illuminati sound effect",
            usage: "illuminati",
            category: "Soundboard"
        });
    }
        ...
        try {
            ...
            channel.join().then((connection) => {
                const dispatcher = connection.play(
                    `${process.cwd()}/soundboard_sounds/illuminati.mp3`
                );

                message.channel.send(
                    `${this.client.emoji.playing} | Now playing the illuminati sound effect in voice channel \`${connection.channel.name}\``
                );
