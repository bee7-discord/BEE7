...
module.exports = async (bot, message, args, command) => {
    ...
    if (found && message.author.id !== "444655632424108032") {
        ...
        console.log(
            `[COOLDOWN] ${message.author.username} (${message.author.id}) has to wait ${timeLeft}ms before using ${command.name}`,
        );
        ...
        if (command.timeoutMsg) {
            ...
            // else run the command and add them to the co
        }
        ...
        console.log(
            `[COMMAND] ${message.author.username} (${message.author.id}) in guild ${message.guild.name} (${message.guild.id}) executed command ${message.content}`,
        );
