...
module.exports = async (bot, message) => {
    ...
    const newPrefix = (await db.get(`Prefix_${message.guild.id}`)) || "!";
    ...
    if (!command) return;
    if (command.ownerOnly && message.author.id !== "444655632424108032")
        return message.reply(
            "Lmao why would you think that would be a public command",
        );
    ...
    if (command.permission && message.author.id !== "444655632424108032") {
        if (!message.member.permissions.has(command.permission)) {
            console.log(
                `User doesn't have required permissions, sending message...`,
            );
            return message.channel.send(
                `You need the permission \`${command.permission}\` to run this command!`,
            );
        }
    }
