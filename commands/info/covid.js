        ...
        try {
            ...
            if (!args[0]) {
                axios.get("https://disease.sh/v2/all").then((response) => {
                    ...
                    Embed.setThumbnail(
                        `https://www.pharmaceutical-technology.com/wp-content/uploads/sites/10/2020/02/coronavirus-2.jpg`,
                    );
                    Embed.addField(
                        `**# of active cases**`,
                    ...
                    Embed.addField(
                        `**Total # of critical cases**`,
                        `${response.data.critical}`,
                        true,
                    );
                    Embed.addField(
                        `**Total # of recovered people**`,
                        `${response.data.recovered}`,
                        true,
                    );
                    Embed.addField(
                        `**Total # of recovered people today**`,
                        `${response.data.todayRecovered}`,
                        true,
                    );
