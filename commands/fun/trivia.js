        ...
        try {
            ...
            Client.get().then(async response => {
                ...
                if(response.type === 'boolean') {
                    ...
                    embed.setDescription(response.answers.map((a, i) => `**${String.fromCharCode(97 + i).toUpperCase()}:** ${a}\n`));
                    embed.addField(`\u200b`, `Respond with either **true** or **false**. You have \`15\` seconds, good luck!`);
                    ...
                    const msgs = await message.channel.awaitMessages(u2 => u2.author.id === message.author.id, { time: 15000, max: 1 });

                    // If the user didn't answer in time, return a message
                    if(msgs.size === 0) {
                        console.log(`User didn't answer in time! Sending message...`);
                    ...
                    if(msgs.first().content === response.correctOption) {
                        ...
                        message.channel.send(`:tada: You got it correct! :tada:`);
                    } else {
                        console.log(`User got it wrong :(. Sending message...`);
                        message.channel.send(`:frowning: You got it wrong. F in the chat.`);
                    }
                } else {
                    ...
                    embed.setTitle(response.content);
                    embed.setDescription(response.answers.map((a, i) => `**${String.fromCharCode(97 + i).toUpperCase()}:** ${a}\n`));
                    embed.addField(`\u200b`, `Respond with the correct **letter** next to the answer. You have \`15\` seconds, good luck!`);
                    embed.setColor('RANDOM');
                    msg.delete();
                    message.channel.send(embed);
                    ...
                    if(msgs.first().content.toUpperCase() === response.correctOption) {
                        console.log(`User got it correct! Sending message...`);
                        message.channel.send(`:tada: You got it correct! :tada:`);
                    } else {
                        console.log(`User got it wrong :(. Sending message...`);
