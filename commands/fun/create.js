        ...
        try {
            ...
            if (!args[0] || !args[1] || !args[2])
                return message.channel.send(
                    `The correct usage for this command is \`${prefix}create <time> <channel mention> <prize>\`!`,
                );

            // If args 0 doesn't end in d, h, m, or s send a message conveying that info
            if (!args[0].match("[dhms]"))
                return message.channel.send(
                    `You did not use the correct formatting for the time! The valid options are \`d\`, \`h\`, \`s\`, or \`m\``,
                );
            ...
            if (args[0] <= 5)
                return message.channels.send(
                    `The giveaway has to last more than 5 seconds!`,
                );
            ...
            const Embed = new MessageEmbed()
                .setTitle(`${prize}`)
                .setDescription(
                    `React with :tada: to enter!\nTime Remaining: **${humanizeDuration(
                        ms(args[0]),
                        { round: true },
            ...
            setTimeout(() => {
                ...
                if (m.reactions.cache.get("🎉").count <= 1) {
                    return m.channel.send(
                        `Not enough people reacted for me to draw a winner! :frowning:`,
                    );
                }
